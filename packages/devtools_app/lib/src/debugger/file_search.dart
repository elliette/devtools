...
class FileSearchFieldState extends State<FileSearchField>
  ...
  void initState() {
    ...
    _query = autoCompleteController.search.trim();
  ...
  FileSearchResults _getSearchResults(
    ...
    for (final scriptRef in scriptRefs) {
      if (fileQuery.isExactFileNameMatch(scriptRef)) {
        exactFileNameMatches.add(scriptRef);
      } else if (fileQuery.isExactFullPathMatch(scriptRef)) {
...
class FileQuery {
  ...
  List<Range> _findFuzzySegments(String fileNameOrPath, String searchQuery) {
    final autoCompleteResultSegments = <Range>[];
    var queryIndex = 0;
    for (int matchIndex = 0; matchIndex < fileNameOrPath.length; matchIndex++) {
      if (queryIndex == query.length) break;
      if (fileNameOrPath[matchIndex] == query[queryIndex]) {
...
class FileSearchResults {
  ...
  FileSearchResults.withQuery({
    @required this.query,
    this.exactFileNameMatches = const [],
  ...
  List<ScriptRef> get scriptRefs => allScripts.isNotEmpty
      ? allScripts
      : [...exactFileNameMatches, ...exactFullPathMatches, ...fuzzyMatches];
